...
if (!fs.existsSync(driverFile)) {
    console.error(`[FATAL] The translation api driver "${process.env.DATABASE_DRIVER}" does not exists.`);
...
const requiredMethods = ['init', 'getLanguage', 'setLanguage', 'addReplicaChannels', 'getReplicaChannels', 'removeReplicaChannels'];
const driver = require(driverFile);
for (const method of requiredMethods) {
    if (!(method in driver)) {
        console.error(`[FATAL] The translation api driver "${process.env.DATABASE_DRIVER}" does not contains a ${method} method.`);
